    ...
    <aside
      ...
      <div class="flex items-center justify-between px-5 py-5 border-b border-dark-700">
        <div class="flex items-center gap-3">
          ...
          <div>
            <span class="font-bold text-white text-sm tracking-tight">Inventário</span>
            <span class="text-primary-400 font-bold text-sm"> Pro</span>
          </div>
        </div>
        <button mat-icon-button class="lg:hidden !text-dark-400" (click)="closeSidebar.emit()">
      ...
      <div class="px-4 py-4 border-b border-dark-700">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-full bg-primary-400/20 flex items-center justify-center flex-shrink-0">
      ...
      <nav class="flex-1 px-3 py-4 space-y-1">
        <p class="px-3 text-xs font-semibold text-dark-500 uppercase tracking-wider mb-3">Menu</p>
        ...
        <p class="px-3 text-xs font-semibold text-dark-500 uppercase tracking-wider mt-5 mb-3">
          Categorias
        </p>
      ...
      <div class="p-3 border-t border-dark-700 space-y-1">
        <button
          ...
          <mat-icon class="!text-lg mr-2">{{ themeService.isDark() ? 'light_mode' : 'dark_mode' }}</mat-icon>
...
export class SidebarComponent {
  ...
  get userName(): string { return this.authService.currentUser()?.nome ?? ''; }
